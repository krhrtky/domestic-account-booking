...
2025-01-15,1000,Test,@SUM(A1)`;
  ...
  describe("Credit Card CSV Pattern Extension (wf-csv-detection-enhancement-20260112)", () => {
    describe("TYP-005: Credit card format with ご利用年月日, ご利用額, ご利用箇所", () => {
      it("detects ご利用年月日 column as date", async () => {
        const { detectHeaders } = await import("./csv-parser");
        const csvContent = `ご利用年月日,ご利用額,ご利用箇所
...
2025-01-15,5400,スーパー`;
      ...
      it("parses real credit card CSV format with all three patterns", async () => {
...
2025/01/20,カフェABC,1500,0`;
        ...
        if (result.success) {
          ...
          expect(result.data[1].description).toBe("カフェABC");
        }
      });
    });

    describe("BND-005: Boundary case with 年月日 pattern only", () => {
      it("detects 年月日 column as date when ご利用年月日 is not present", async () => {
        const { detectHeaders } = await import("./csv-parser");
        const csvContent = `年月日,金額,摘要
2025-01-15,5400,スーパー`;
        ...
        expect("error" in result).toBe(false);
        if ("headers" in result) {
          expect(result.suggestedMapping.dateColumn).toBe("年月日");
        }
      });

      it("detects 利用額 column as amount when ご利用額 is not present", async () => {
        const { detectHeaders } = await import("./csv-parser");
        const csvContent = `日付,利用額,摘要
2025-01-15,5400,スーパー`;
        ...
        expect("error" in result).toBe(false);
        if ("headers" in result) {
          expect(result.suggestedMapping.amountColumn).toBe("利用額");
        }
      });

      it("detects 利用箇所 column as description when ご利用箇所 is not present", async () => {
        const { detectHeaders } = await import("./csv-parser");
        const csvContent = `日付,金額,利用箇所
2025-01-15,5400,スーパー`;
        ...
        expect("error" in result).toBe(false);
        if ("headers" in result) {
          expect(result.suggestedMapping.descriptionColumn).toBe("利用箇所");
        }
      });
    });

    describe("INC-001: Regression test for user-reported bug", () => {
      it('does not show "必須列が見つかりません" error for credit card CSV', async () => {
        const csvContent = `ご利用年月日,ご利用箇所,ご利用額
2025/01/15,スーパーXYZ,10000`;

        const result = await parseCSV(csvContent, "credit-card.csv");
