      ...
      complete: (results) => {
        ...
        if (results.data.length > MAX_ROW_COUNT) {
          resolve({
            success: false,
            errors: [
              `行数が上限(${MAX_ROW_COUNT.toLocaleString()}行)を超えています`,
            ],
          });
          return;
        }
        ...
        const { filteredHeaders, excludedColumns } =
          filterSensitiveColumns(headers);
        const warnings: string[] = [];

        if (excludedColumns.length > 0) {
          warnings.push(
            `機密情報を含む可能性のある列を除外しました: ${excludedColumns.join(", ")}`,
          );
        }
        ...
        if (!dateColumn || !descColumn || !amountColumn) {
          const headerList =
            filteredHeaders.length > 0 ? filteredHeaders.join(", ") : "なし";
          resolve({
            success: false,
            errors: [
              "必須列（日付、金額、摘要）が見つかりません。検出された列: " +
                headerList,
            ],
