...
console.error(
  `[DEBUG] DATABASE_URL: ${databaseUrl ? "set (length: " + databaseUrl.length + ")" : "not set"}`,
);
console.error(
  `[DEBUG] NEXTAUTH_SECRET: ${nextAuthSecret ? "set (length: " + nextAuthSecret.length + ")" : "not set"}`,
);
...
async function createTestSession() {
  ...
  try {
    ...
    await client.query("DELETE FROM auth_users WHERE email = $1", [testEmail]);

    console.error("[DEBUG] Hashing password...");
    const passwordHash = await bcrypt.hash(testPassword, 12);

    console.error("[DEBUG] Creating auth_users record...");
