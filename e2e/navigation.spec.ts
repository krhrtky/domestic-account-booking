...
test.describe("Navigation Component", () => {
  ...
  test.describe("AC-NAV-007: Navigation items", () => {
    test("displays all navigation items", async ({ page }) => {
      ...
      await page.goto("/dashboard");
      await expect(
        page.locator("nav").getByRole("link", { name: "ダッシュボード" }),
      ).toBeVisible();
      await expect(
        page.locator("nav").getByRole("link", { name: "取引一覧" }),
      ).toBeVisible();
      await expect(
        page.locator("nav").getByRole("link", { name: "グループ設定" }),
      ).toBeVisible();
      await expect(
  ...
  test.describe("AC-NAV-008: Active state highlighting", () => {
    ...
    test("highlights active navigation item on transactions page", async ({
      page,
    }) => {
    ...
    test("highlights active navigation item on settings page", async ({
      page,
    }) => {
  ...
  test.describe("Mobile Navigation", () => {
    ...
    test("opens mobile menu and shows navigation items", async ({ page }) => {
      ...
      await expect(
        page.getByRole("link", { name: "取引一覧" }).first(),
      ).toBeVisible();
      await expect(
        page.getByRole("link", { name: "グループ設定" }).first(),
      ).toBeVisible();
    });
