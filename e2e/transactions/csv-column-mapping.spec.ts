...
test.describe("CSV Column Mapping", () => {
  ...
  test("UC-001: Auto-detects standard headers and allows mapping verification", async ({
    page,
  }) => {
...
2025-01-16,450,Coffee Shop`;
  ...
  test("UC-002: Manual column mapping for non-standard headers", async ({
    page,
  }) => {
...
2025-01-15,5400,Test,1234-5678-9012-3456`;
    ...
    await expect(
      page.getByText("機密情報を含む可能性のある列を除外しました: カード番号"),
    ).toBeVisible();
  });
...
2025-01-15,Test Item`;
  ...
  test("L-CX-004: UI responds within 200ms on button click", async ({
    page,
  }) => {
    const csvContent = `Date,Amount,Description
2025-01-15,100,Test`;
  ...
  test("L-BR-006: Auto-detects payer column and displays in preview", async ({
    page,
  }) => {
...
2025-01-17,3200,Dinner,—`;
    ...
    await expect(
      page.getByRole("columnheader", { name: "支払者(CSV)" }),
    ).toBeVisible();
...
2025-01-16,450,Coffee`;
  ...
  test("L-SC-002: Payer field formula injection is sanitized", async ({
    page,
  }) => {
    const csvContent = `Date,Amount,Description,Payer
