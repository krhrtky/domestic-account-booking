...
test.describe.skip("Scenario 15: Edge Cases & Data Boundaries", () => {
  ...
  test("should handle zero transactions gracefully", async ({ page }) => {
    ...
    await expect(
      page
        .locator('[data-testid="settlement-summary"]')
        .getByText(/0|今月の取引はありません/),
    ).toBeVisible();
  });
  ...
  test("should handle exactly equal contributions", async ({ page }) => {
    ...
    if (await monthSelector.isVisible()) {
      ...
      const otherMonth = options.find(
        (opt) => !opt.includes(currentMonth.split("-")[1]),
      );
      if (otherMonth) {
        const otherValue = await monthSelector
          .locator(`option:has-text("${otherMonth}")`)
          .getAttribute("value");
        if (otherValue) {
    ...
    await expect(
      page.locator('[data-testid="settlement-summary"]'),
  ...
  test("should handle single transaction", async ({ page }) => {
    ...
    if (await monthSelector2.isVisible()) {
      ...
      if (otherMonth) {
        const otherValue = await monthSelector2
          .locator(`option:has-text("${otherMonth}")`)
          .getAttribute("value");
        if (otherValue) {
  ...
  test("should handle very small amounts", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="transaction-amount"]').getByText("50"),
