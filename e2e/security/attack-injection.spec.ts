...
test.describe("L-SC-002, L-TA-001: インジェクション攻撃シナリオ (Attack Cases)", () => {
  ...
  test.beforeAll(async () => {
    ...
    groupId = await createTestGroup(testUser.id!, {
      name: "XSS Test Group",
      ratioA: 50,
      ratioB: 50,
    });
  ...
  test.describe.serial("ATK-003: XSS防御（取引摘要）", () => {
    test("手動入力の取引摘要にスクリプトタグを含めても実行されない", async ({
      page,
    }) => {
      ...
      const hasExecutableScript = await page
        .locator('script:has-text("alert(document.cookie)")')
        .count();
    ...
    test("javascript: スキームを含むリンクも無害化される", async ({ page }) => {
      ...
      const hasJavascriptLink = await page
  ...
  test.describe.skip("ATK-004: CSVインジェクション防御", () => {
    test("数式インジェクション（=CMD）を含むCSVをアップロードしてもエスケープされる", async ({
      page,
    }) => {
      await loginUser(page, testUser);
...
2025-01-16,2000,正常な摘要`;
      ...
      const csvPath = path.join(
        __dirname,
        "../../tests/fixtures/csv-injection-test.csv",
      );
      const fs = require("fs");
      ...
      await expect(
        page.locator('h2:has-text("列マッピングの確認")'),
      ...
      const injectionTransaction = transactions.data?.find(
      ...
      const hasSingleQuotePrefix =
        injectionTransaction.description.startsWith("'=");
      expect(hasSingleQuotePrefix || !startsWithEquals).toBeTruthy();

      fs.unlinkSync(csvPath);
    });

    test("ハイパーリンク数式（=HYPERLINK）を含むCSVもエスケープされる", async ({
      page,
    }) => {
      await loginUser(page, testUser);
...
2025-01-19,6000,正常な摘要`;
      ...
      const csvPath = path.join(
        __dirname,
        "../../tests/fixtures/csv-hyperlink-test.csv",
      );
      const fs = require("fs");
      fs.writeFileSync(csvPath, csvContent, "utf-8");
      ...
      const hyperlinkTransaction = transactions.data?.find(
        (t: { description: string }) =>
          t.description.includes("HYPERLINK") ||
          t.description.includes("evil.com"),
      );
...
2025-01-22,3000,@SUM(A1:A2)`;
      ...
      const csvPath = path.join(
        __dirname,
        "../../tests/fixtures/csv-formula-test.csv",
      );
      const fs = require("fs");
      fs.writeFileSync(csvPath, csvContent, "utf-8");
      ...
      const formulaTransactions = transactions.data?.filter(
        (t: { description: string }) => /^[\+\-@]/.test(t.description),
      );
