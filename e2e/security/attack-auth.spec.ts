...
test.describe("L-SC-001, L-TA-001: 認証攻撃シナリオ (Attack Cases)", () => {
  test.describe("ATK-001: 未認証ページアクセス防御", () => {
    ...
    test("未認証で/dashboardにアクセスすると/loginへリダイレクトされる", async ({
      page,
    }) => {
    ...
    test("未認証で/dashboard/transactionsにアクセスすると/loginへリダイレクトされる", async ({
      page,
    }) => {
    ...
    test("未認証で/settingsにアクセスすると/loginへリダイレクトされる", async ({
      page,
    }) => {
    ...
    test("未認証で/dashboard/transactions/uploadにアクセスすると/loginへリダイレクトされる", async ({
      page,
    }) => {
  ...
  test.describe.skip("ATK-002: IDOR（他グループデータアクセス拒否）", () => {
    ...
    test("攻撃者が他グループのトランザクション詳細ページにアクセスすると403またはリダイレクト", async ({
      page,
      request,
    }) => {
      ...
      if (victimTransactionId) {
        const response = await request.get(
          `/api/transactions/${victimTransactionId}`,
          {
            headers: {
              Cookie: await page
                .context()
    ...
    test("攻撃者がAPIで他グループのトランザクション一覧を取得しようとすると自グループのみ返る", async ({
      page,
    }) => {
      await loginUser(page, attackerUser);
